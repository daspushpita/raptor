...
double Temperature1(double rho, double tracer, double pp, double gammarel, double X_u[4], double U_u[4]) {
...
void star_BB_emission(double *lightpath, int steps,
    ...
    if (get_fluid_params_star(X_u, &modvar)) {
        ...
        Temp = 1.e5;
        if (modvar.tr1 >= 0.5){
            Temp = 1.e7;//Temperature1(modvar.prim_rho, modvar.tr1, modvar.prim_pp, modvar.gammarel, X_u, modvar.U_u);
        }        
        for (int f = 0; f < num_frequencies; f++) {
            ...
            redshift = sqrt(1.0 - 2.0/radii);
            ini_freq = frequency[f]/redshift;
	    if (X_u[3]>=0.){
            	IQUV[f][0] = BB_spectrum(ini_freq, Temp)/(ini_freq * ini_freq * ini_freq);
	    }
